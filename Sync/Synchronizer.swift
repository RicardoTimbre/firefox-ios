...
class BaseSingleCollectionSynchronizer: SingleCollectionSynchronizer {
...
class ClientsSynchronizer: BaseSingleCollectionSynchronizer, Synchronizer {
    required init(scratchpad: Scratchpad, basePrefs: Prefs) {
    ...
    func synchronizeLocalClients(localClients: Clients, withServer storageClient: Sync15StorageClient, info: InfoCollections) -> Deferred<Result<()>> {
...
class TabsSynchronizer: BaseSingleCollectionSynchronizer {
    required init(scratchpad: Scratchpad, basePrefs: Prefs) {
        super.init(scratchpad: scratchpad, basePrefs: basePrefs, collection: "tabs")
    }

    func synchronizeLocalTabs(localTabs: RemoteClientsAndTabs, withServer storageClient: Sync15StorageClient, info: InfoCollections) -> Deferred<Result<()>> {
        if !self.remoteHasChanges(info) {
            // Nothing to do.
        ...
        if let factory: (String) -> TabsPayload? = self.scratchpad.keys?.value.factory(self.collection, f: { TabsPayload($0) }) {
            ...
            return chainDeferred(tabsClient.getSince(self.lastFetched), {
                ...
                for (record) in records {
                    ...
                    localTabs.insertOrUpdateTabsForClient(record.id, tabs: record.payload.remoteTabs)
                }
